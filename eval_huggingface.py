...
if __name__ == '__main__':
    ...
    trainer, training_args = init_trainer(model=model,
                                ...
                                val_dataset=tokenized_datasets['validation'] if args.dataset_name!='thaiqa_squad' else tokenized_datasets['train'],
                                args=args,
                                data_collator=data_collator,
                                tokenizer=tokenizer,)
    ...
    tokenizer = AutoTokenizer.from_pretrained("zhufy/xquad-th-bert-base")
    model = AutoModelForQuestionAnswering.from_pretrained("zhufy/xquad-th-bert-base")
    trainer, training_args = init_trainer(model=model,
                                train_dataset=tokenized_datasets['train'],
                                val_dataset=tokenized_datasets['validation'] if args.dataset_name!='thaiqa_squad' else tokenized_datasets['train'],
                                args=args,
    ...
    result_word,_,_ = question_answering_metrics(datasets=datasets['test'] if args.dataset_name!='thaiqa_squad' else datasets['validation'],
                                        trainer=trainer,
    ...
    result_syllable,_,_ = question_answering_metrics(datasets=datasets['test'] if args.dataset_name!='thaiqa_squad' else datasets['validation'],
                                        trainer=trainer,
                                        metric=squad_newmm_metric,
    ...
    result_character,_,_ = question_answering_metrics(datasets=datasets['test'] if args.dataset_name!='thaiqa_squad' else datasets['validation'],
                                        trainer=trainer,
                                        metric=squad_newmm_metric,
